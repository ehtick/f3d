...
f3d_test(NAME TestPNG DATA world.png ARGS --camera-orthographic)
f3d_test(NAME TestJPG DATA world.jpg ARGS --camera-orthographic)
f3d_test(NAME TestBMP DATA albedo.bmp ARGS --camera-orthographic)
f3d_test(NAME TestTGA DATA world.tga ARGS --camera-orthographic)
f3d_test(NAME TestHDR DATA palermo_park_1k.hdr ARGS --camera-orthographic)
if(F3D_MODULE_WEBP)
  f3d_test(NAME TestWebP DATA image.webp ARGS --camera-orthographic)
endif()
if(F3D_MODULE_EXR)
  f3d_test(NAME TestEXR DATA Rec709.exr ARGS --camera-orthographic)
endif()
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.4.20250501)
  f3d_test(NAME TestPipedPLY DATA suzanne.ply PIPED PLYReader)
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251104)
  f3d_test(NAME TestPipedSTL DATA suzanne.stl PIPED STL)
  if(NOT F3D_MACOS_BUNDLE)
    f3d_test(NAME TestDefaultConfigFilePipedSTL DATA suzanne.stl PIPED STL CONFIG config_build LONG_TIMEOUT UI)
  endif()
endif()
...
if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251210)
  f3d_test(NAME TestPipedDicom DATA IM-0001-1983.dcm ARGS --scalar-coloring --roughness=1 PIPED DICOM)
  f3d_test(NAME TestPipedPNG DATA world.png ARGS --camera-orthographic PIPED PNG)
  f3d_test(NAME TestPipedJPG DATA world.jpg ARGS --camera-orthographic PIPED JPEG)
  f3d_test(NAME TestPipedBMP DATA albedo.bmp ARGS --camera-orthographic PIPED BMP)
  f3d_test(NAME TestPipedTGA DATA world.tga ARGS --camera-orthographic PIPED TGA)
  f3d_test(NAME TestPipedHDR DATA palermo_park_1k.hdr ARGS --camera-orthographic PIPED HDR)
  if(F3D_MODULE_WEBP)
    f3d_test(NAME TestPipedWebP DATA image.webp ARGS --camera-orthographic PIPED WebP)
  endif()
  if(F3D_MODULE_EXR)
    f3d_test(NAME TestPipedEXR DATA Rec709.exr ARGS --camera-orthographic PIPED EXR)
  endif()
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.6.20260103)
  f3d_test(NAME TestPipedOBJ DATA world.obj PIPED OBJ) # MTL and Textures not loaded
  f3d_test(NAME TestPipedVTK DATA cow.vtk PIPED VTKLegacy)
endif()

if(VTK_VERSION VERSION_GREATER_EQUAL 9.6.20260106)
  f3d_test(NAME TestPipedVTR DATA RectGrid2.vtr ARGS --scalar-coloring --roughness=1 PIPED VTKXMLVTR)
  f3d_test(NAME TestPipedVTU DATA dragon.vtu PIPED VTKXMLVTU)
  f3d_test(NAME TestPipedVTP DATA cow.vtp PIPED VTKXMLVTP)
  f3d_test(NAME TestPipedVTS DATA bluntfin.vts PIPED VTKXMLVTS)
  f3d_test(NAME TestPipedVTICell DATA waveletMaterial.vti ARGS -s --coloring-array=Material -c --roughness=1 PIPED VTKXMLVTI)
endif()
...
if(NOT F3D_MACOS_BUNDLE)
  ...
  if(VTK_VERSION VERSION_GREATER_EQUAL 9.5.20251001)
    ...
    if(F3D_MODULE_WEBP)
      f3d_test(NAME TestDefaultConfigFileWebP DATA image.webp CONFIG config_build LONG_TIMEOUT TONE_MAPPING UI)
    endif()
    if(F3D_MODULE_EXR)
      f3d_test(NAME TestDefaultConfigFileEXR DATA Rec709.exr CONFIG config_build LONG_TIMEOUT TONE_MAPPING UI)
    endif()
    f3d_test(NAME TestDefaultConfigFilePLY DATA suzanneRGBA.ply CONFIG config_build LONG_TIMEOUT TONE_MAPPING UI)
    f3d_test(NAME TestDefaultConfigFileQuakeMDL DATA zombie.mdl CONFIG config_build LONG_TIMEOUT TONE_MAPPING UI THRESHOLD 0.07) # Threshold is needed for IBL change after VTK 9.6
  ...
  if(VTK_VERSION VERSION_GREATER_EQUAL 9.4.20250118)
    ...
    if(F3D_MODULE_EXR)
      f3d_test(NAME TestThumbnailConfigFileEXR DATA Rec709.exr CONFIG thumbnail_build LONG_TIMEOUT TONE_MAPPING)
    endif()
  endif()
  if(F3D_MODULE_WEBP)
    f3d_test(NAME TestThumbnailConfigFileWebP DATA image.webp CONFIG thumbnail_build LONG_TIMEOUT TONE_MAPPING)
  endif()
